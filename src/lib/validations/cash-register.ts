...
export const openCashRegisterSchema = z.object({
	...
	openingBillCounts: z.record(z.string(), z.number().int().min(0)).optional().default({}),
	openingNotes: z.string().optional()
});

// Esquema para cierre de caja
export const closeCashRegisterSchema = z
	.object({
		actualAmount: z.number().min(0, 'El monto real debe ser mayor o igual a 0'),
		notes: z.string().optional(),
		billCounts: z.record(z.string(), z.number().int().min(0)).optional().default({})
	})
	.refine(
		(data) => {
